...
def Captured_GPIO(e):
    ...
    for i in range(0,int(CapturedTimes)):
        ...
        # time.sleep(0.5)    # 把USB传输数据前的那段时间等过去,不能让GPIO的中断捕获跑到USB传输前面去
...
if __name__ == "__main__":
    ...
    write_flag = multiprocessing.Event()

    task_Captured_GPIO = multiprocessing.Process(name = 'block', target = Captured_GPIO, args = (e,))
    task_USB_Sample = multiprocessing.Process(name = 'block', target = USB_Sample, args = (e,write_flag,))
    # task_GDB_Command = multiprocessing.Process(name = 'block', target = GDB_Command, args = (e,))
    # task_USB_Sample_OS = multiprocessing.Process(name = 'block', target = USB_Sample_OS, args = (e,))
    # task_Write_file = multiprocessing.Process(name = 'block', target = Write_file, args = (write_flag,))
    ...
    # task_Write_file.start()
    
    time.sleep(0.9)   # 等1秒钟让他们跑到该跑的位置上
    ...
    # gdb.execute('monitor resume')    # 现在遇到的问题是写入数据时会卡死，可以考虑起全局变量，然后放到main或专门起一个进程写入数据
